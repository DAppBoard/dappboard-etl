...
to_timestamp(   SUBSTRING(split_part(encode(decode(SUBSTRING("data", 1, 42), 'hex'), 'escape'), '.', 3), 1, 10)::Integer )::timestamp AS "flight_departure",
hex2dec(SUBSTRING("data", 65, 64))::INTEGER AS "premium",
 hex2dec((SUBSTRING("data", 129, 64)))::INTEGER AS "indemnity" ,
...
to_timestamp(   SUBSTRING(split_part(encode(decode(SUBSTRING("data", 65, 42), 'hex'), 'escape'), '.', 3), 1, 10)::Integer )::timestamp AS "flight_departure",
hex2dec(SUBSTRING("data", 129, 64))::INTEGER AS "premium",
 hex2dec((SUBSTRING("data", 193, 64)))::INTEGER AS "indemnity" ,
